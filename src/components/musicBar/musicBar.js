  ...
  const handleSongPage = (index, array) => {
    ...
    if (screenSize.current < 979) {
      ...
      if (array.length !== 0) {
        ...
        return navigate(
          `/songDetailPage/${array.length != 0 ? array[index].songId : "13a"}`
        );
      } else {
      ...
      <div className="live_music">
        <div className={!pathname.includes("queue") ? "leftb" : "leftm-bar"}>
          <img
            src={
              songArr.length !== 0
                ? songArr[currentTrackIndex].image
                : "https://c.saavncdn.com/973/Vikram-Tamil-2022-20220515182605-500x500.jpg"
            }
            width="50px"
            height="50px"
            alt="song_logo"
            onClick={() => handleSongPage(currentTrackIndex, songArr)}
          />
          <p>
            {songArr.length !== 0
              ? songArr[currentTrackIndex].title
              : "Once upon a time"}
            <br />
            <span className="describe">
              {songArr.length != 0
                ? songArr[currentTrackIndex].artist
        ...
        <div
          className={
            screenSize.current < 979 && pathname.includes("queue")
              ? "queuebar"
              : "midb"
          }
        >
          ...
          <BiRepeat
            className="plyrepeat"
            id={isLoopOn ? "plyRepeat" : "noRepeat"}
          ...
          <p className="plysong">
            {playing ? (
              <GiPauseButton onClick={handlePause} />
            ) : (
              <TbPlayerPlayFilled onClick={handlePlay} />
            )}
          </p>
          ...
          />
          <PiShuffle
            className="plyshuffle"
            id={isShuffleOn ? "shuffle" : "noShuffle"}
        ...
        <div className="rightb">
          ...
          <Link
            to={`/songDetailPage/${
              songArr.length !== 0 ? songArr[currentTrackIndex].songId : "13a"
            }`}
          >
            {" "}
            <TfiMoreAlt className="more-btn" />
          </Link>
          <div className="volume-sec">
            ...
            {volume !== 0 ? (
              <RiVolumeUpFill className="volum-btn" onClick={handleMute} />
            ) : (
              <MdVolumeOff className="volum-btn" />
            )}
          </div>
          {expand ? (
            <Link to={`${path.current}`}>
              <GrContract onClick={handleShrink} style={{ color: "#3e3e3e" }} />
            </Link>
          ) : (
            <Link to="/queue">
              <CgArrowsExpandRight
