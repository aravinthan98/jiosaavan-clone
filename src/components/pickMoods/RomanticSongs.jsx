...
function RomanticMoodSongs(){
    ...
    const handlePlayMoods=(e,id)=>{
      e.stopPropagation();
      setSongArr(romanticMood)
      const curIndex=romanticMood.findIndex(object => {
...
const handleSongPage=(id,song)=>{
  setSongObject(song)
    ...
    return(
        ...
        >
            {romanticMood.length===0?(<LoaderFn/>):(
                    romanticMood.map((item)=>(
...
export default RomanticMoodSongs;